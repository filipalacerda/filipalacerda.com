...
export default function Talks() {
  ...
  const tabsTitles = ["Talks", "Podcasts"];
  ...
  return (
    <section className="mt-12">
      <ul className="tabs flex mb-4 fixed pt-4 top-0 z-10 w-full">
        {tabsTitles.map((tabTitle, index) => (
          <TabTitle
            title={tabTitle}
            key={tabTitle}
            setSelectedTab={() => setSelectedTab(index)}
            cssClass={`${index === selectedTab && "underline"} mr-4 text-md font-semibold text-orangeDark`}
            isActive={index === selectedTab}
          />
        ))}
      </ul>
      <div className="relative mt-4">
        <Tab content={tabsContent[selectedTab]} />
      </div>
    </section>
  );
}
